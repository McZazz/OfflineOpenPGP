import { makeElement, Component, appendMultiple, roundToTwo } from './utils.js';
...
export class ScrollBar extends Component {
	constructor({
		...
		this.track = makeElement({type:'div', style:this.track_styles});

		// create bar element
		this.bar = makeElement({type:'div', style:this.bar_styles});
		...
		if (end_blocks === true) {
			this.start_block = makeElement({
				...
				style:[
					...
					[this.width_side, '100%'],
					// ['background', this.global_styles['colors']['panel_dark_bg']]
				]
			});
    ...
    // 	console.log('rect', rect.top, rect.bottom, 'scrollcont', this.container_size.top, this.container_size.bottom, 'scroll', this.scrolldrawer_size.top, this.scrolldrawer_size.bottom);
    // }
  ...
  // 	this.external_drag_initiator.forceCardUnderMouse();
	...
	hideBar = () => {
		if (this.scroll_hides) {
			...
			this.scroll_drawer.style.top = '0px'; /////////////////// possibly only works on dialog scrolls...?
		}
	}
